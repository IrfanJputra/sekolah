var ParallaxManager, ParallaxPart, yall = function() {
    "use strict";
    return function(c) {
...
ParallaxPart = function() {
        function a(a) {
            this.el = a, this.speed = parseFloat(this.el.getAttribute("data-parallax-speed")), this.maxScroll = parseInt(this.el.getAttribute("data-max-scroll"))
        }
        return a.prototype.update = function(a) {
            if (!(a > this.maxScroll)) {
                var b = -(a * this.speed);
                this.setYTransform(b)
            }
        }, a.prototype.setYTransform = function(a) {
            this.el.style.webkitTransform = "translate3d(0, " + a + "px, 0)", this.el.style.MozTransform = "translate3d(0, " + a + "px, 0)", this.el.style.OTransform = "translate3d(0, " + a + "px, 0)", this.el.style.transform = "translate3d(0, " + a + "px, 0)", this.el.style.msTransform = "translateY(" + a + "px)"
        }, a
    }(), ParallaxManager = function() {
        function a(a) {
            if (Array.isArray(a) && a.length && (this.elements = a), "object" == typeof a && a.item) this.elements = Array.prototype.slice.call(a);
            else if ("string" == typeof a) {
                if (this.elements = document.querySelectorAll(a), 0 === this.elements.length) throw new Error("Parallax: No elements found");
                this.elements = Array.prototype.slice.call(this.elements)
            } else throw new Error("Parallax: Element variable is not a querySelector string, Array, or NodeList");
            for (var b in this.elements) this.parts.push(new ParallaxPart(this.elements[b]));
            window.addEventListener("scroll", this.onScroll.bind(this))
        }
        return a.prototype.parts = [], a.prototype.onScroll = function() {
            window.requestAnimationFrame(this.scrollHandler.bind(this))
        }, a.prototype.scrollHandler = function() {
            var a = Math.max(window.pageYOffset, 0);
            for (var b in this.parts) this.parts[b].update(a)
        }, a
    }(), ! function(q, a, b) {
...
get_header(), $("a[href=\"#search_popup\"]").click(function(a) {
    ...
    $("#fab .fab_target a").on("click", function(a) {
        var b = $(this.getAttribute("href")),
            c = $(this).attr("href");
        "javascript:void(0)" != c && ($("body").removeClass("noscroll"), $(".overlay").removeClass("is-active"), 0 == b.length ? $("html, body").animate({
            scrollTop: 0
        }, 550) : $("html, body").stop().animate({
            scrollTop: b.offset().top - 36
        }, 550)), a.preventDefault()
    ...
    $("#kritiksaran").on("submit", function(a) {
        a.preventDefault();
        let b = new FormData($("#kritiksaran")[0]);
        $.ajax({
            url: BASE_URL + "beranda/kritiksaran",
            type: "POST",
            data: b,
            dataType: "json",
            cache: !1,
            ...
            contentType: !1,
            beforeSend: function() {
                $("#submit").prop("disabled", !0), $("#alert_kritiksaran").html(""), $(".spinner").show()
            },
            success: function(a) {
                "success" === a.type ? ($("#submit").prop("disabled", !1), $("#alert_kritiksaran").hide().html(a.message).slideDown("slow"), $("#kritiksaran")[0].reset(), $(".spinner").hide(), setTimeout(() => {
                    $("#alert_kritiksaran").slideUp("slow")
                }, 5e3), $(".spinner").hide()) : "danger" === a.type ? ($("#submit").prop("disabled", !1), $("#alert_kritiksaran").hide().html(a.message).slideDown("slow"), $(".spinner").hide()) : "hack" === a.type && ($("#alert_kritiksaran").hide().html(a.message).slideDown("slow"), $("#kritiksaran")[0].reset(), $(".spinner").hide())
            },
            error: function(a) {
                $("#submit").prop("disabled", !1), $("#alert_kritiksaran").hide().html(a.responseText).slideDown("slow"), $(".spinner").hide()
            }
        })
    })
});